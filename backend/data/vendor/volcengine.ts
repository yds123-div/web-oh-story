...
const vendor: VendorConfig = {
  ...
  inputs: [
    ...
    { key: "baseUrl", label: "请求地址", type: "url", required: true, placeholder: "以v3结束，示例：https://ark.cn-beijing.volces.com/api/v3 或 Agent Plan：https://ark.cn-beijing.volces.com/api/plan/v3" },
  ],
  inputValues: {
    apiKey: "",
  ...
  models: [
    ...
    // ===================== 图片生成模型 =====================
    {
      name: "Seedream-5.0-Pro(Agent Plan)",
    ...
    // ===================== 视频生成模型 =====================
    {
      name: "Seedance-1.5-Pro(Agent Plan)",
      modelName: "doubao-seedance-1.5-pro-即将下线",
      type: "video",
...
const imageRequest = async (config: ImageConfig, model: ImageModel): Promise<string> => {
  ...
  const is5Lite = /seedream-5[.\-]0-lite/i.test(model.modelName);
  // Agent Plan 的 seedream-5.0-pro 等模型不支持该参数；仅 5.0-lite / 4.5 / 4.0 支持
  const supportsSequential = is5Lite || /seedream-4[.\-][05]/i.test(model.modelName);
